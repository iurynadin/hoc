class ButtonTextPlusElement extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const leading = (this.getAttribute('leading') ? this.getAttribute('leading') : 'leading-tight' );
        const poslabel = (this.getAttribute('poslabel') ? this.getAttribute('poslabel') : '' );

        this.innerHTML = `
            <div class="group hover:cursor-pointer flex justify-start items-center transition-all duration-300">
                <div class="mr-[3px] uppercase ${textColor} font-gotham_bold text-base md:text-lg xl:text-[20px] ${leading} group-hover:text-amarelopastel transition-all duration-300">${this.getAttribute('label')}${poslabel}</div>
